...
require([
    ...
    function addRow(e) {
        ...
        var size = store.data.length - 1;

        // increase ID += 1, to give room to new row
        for (var i = size; i >= 0; i--) {
            var cenas = store.getSync(i);
            cenas.id += 1
            store.put(cenas);
        };

        store.put(new grid_line(0, "NEW"));
        grid.refresh();
        doIndentation(store.data);
    }
    ...
    function doPrettify(e) {
        ...
        // // store.data = _data;
        // store.sort("project", "ascending");

        // grid.collection.sort("project");
        grid.refresh();
        doIndentation(store.data);
    }
